...
for b=1:50
    ...
    w1=Wn(sig2,200); %Generating the noise
    ...
    for i=1:50
        ...
        u1(i,b)=Usm(xs1,i,200,sig2,110); %Calculating the Usm
        l1(i,b)=Lsm(xs1,i,200,sig2,110); %Calculating the Lsm
        zsu1(i,b)=zsm_u(u1(i,b),200,i,sig2,110); %calculating the upper bound of Reconstruction erro
        zsl1(i,b)=zsm_l(l1(i,b),200,i,sig2,110); %calculating the lower bound of Reconstruction erro
    ...
    f1=mean(J');    %Data error
    f3=mean(p0'); %the Zsm bound for p1=p2=0
    f4=mean(jsu2'); %the upper bound for data error
    f5=mean(zsl1'); %the lower bond for reconstruction error Zsm
    f6=mean(jsl2');%%the lower bound for data error
z=1:1:50;
